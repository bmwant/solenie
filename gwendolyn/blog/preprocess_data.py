...
def get_dataset_1M():
    ...
    training_set = pd.read_csv('./data/ml-1m/train.dat', sep='::', header=None, engine='python', encoding='latin-1')
    training_set = np.array(training_set, dtype=np.uint32)

    test_set = pd.read_csv('./data/ml-1m/test.dat', sep='::', header=None, engine='python', encoding='latin-1')
...
def get_dataset_100k():
    training_set = pd.read_csv('./data/ml-100k/ua.base', delimiter='\t')
    training_set = np.array(training_set, dtype=np.uint32)

    test_set = pd.read_csv('./data/ml-100k/ua.test', delimiter='\t')
    test_set = np.array(test_set, dtype=np.uint32)
